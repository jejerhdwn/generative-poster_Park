...
st.set_page_config(page_title="Pastel Stars App", layout="centered")
st.header("Pastel Stars Generator")
...
seed_val = st.sidebar.number_input("Seed", min_value=0, max_value=10000000, value=42, step=1, disabled=not use_seed)
...
fig = render(
    ...
    seed=seed if (use_seed or regen) else None,
)
st.pyplot(fig, clear_figure=True)
plt.close(fig)
st.caption("If you still see a blank page, hard refresh the browser (Ctrl/Cmd + Shift + R).")
